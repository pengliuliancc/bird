!function(){
  ...
  $('.actions').on('click', 'button', function(e){
    let $button = $(e.currentTarget) // button dom事件对象的属性currentTarget指当事件遍历dom时，标识事件当前的目标。e.currentTarget返回的是监听的节点。
    let speed = $button.attr('data-speed')
    $button.addClass('active')
      .siblings('.active').removeClass('active')//jQuery：  .siblings( )遍历方法 查找  .removeClass 移除一个或者多个类
    switch(speed){
      case 'slow':
        duration = 100
        break
